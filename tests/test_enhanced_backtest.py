...
class TestSlippageModels:
    ...
    def test_depth_based_slippage(self):
        ...
        snapshot = SlippageOrderBookSnapshot(
            symbol="BTC/USDT", timestamp=datetime.now(), bids=bids, asks=asks
        )

        # Test slippage calculation
        slippage_calc = DepthBasedSlippage()
        context = SlippageContext(
            symbol="BTC/USDT", side="buy", quantity=2.0, order_type="market", order_book=snapshot
        )
...
class TestOrderBookSimulation:
    ...
    def test_order_book_snapshot_validation(self):
        ...
        valid_snapshot = OrderBookSnapshot(
            symbol="BTC/USDT", timestamp=datetime.now(), bids=valid_bids, asks=valid_asks
        )
        ...
        invalid_snapshot = OrderBookSnapshot(
            symbol="BTC/USDT", timestamp=datetime.now(), bids=invalid_bids, asks=invalid_asks
        )
    ...
    def test_order_simulation(self):
        ...
        order = SimulatedOrder(
            order_id="test_1", symbol="BTC/USDT", side="buy", order_type="market", quantity=2.0
        )
...
class TestIntegration:
    ...
    def test_fee_slippage_integration(self):
        ...
        slippage_context = SlippageContext(
            symbol="BTC/USDT", side="buy", quantity=0.2, order_type="market", order_book=snapshot
        )
    ...
    def test_order_book_replay_integration(self):
        ...
        # Replay snapshots
        replayed_snapshots = list(
            replay_engine.replay_snapshots(
                "BTC/USDT", snapshots[0].timestamp, snapshots[-1].timestamp
            )
        )
