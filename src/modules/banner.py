...
def render_banner(tracker) -> None:
    ...
    try:
        ...
        providers_active = ",".join(
            sorted(list(getattr(tracker.aggregator, "enabled_sources", {"cmc"})))
        )
        ttl = getattr(tracker, "ttl_seconds", 15)
        agreement = getattr(tracker, "agreement_max_diff_pct", 0.5)
        console.print(
            f"Providers: {providers_active} | TTL(s): {ttl} | Agreement max diff(%): {agreement}"
        )
    except Exception:
        pass
    ...
    try:
        ...
        ds = (
            float(tracker._daily_equity_start_usd)
            if getattr(tracker, "_daily_equity_start_usd", None) is not None
            else 0.0
        )
        dd = 0.0
        if ds > 0:
        ...
        console.print(
            f"[blue]Equity:[/] ${eq:,.2f} | [blue]DayStart:[/] ${ds:,.2f} | [blue]DD:[/] {dd:.2f}%"
        )
    except Exception:
        pass
    ...
    try:
        ...
        if queue:
            ...
            console.print(
                f"[blue]Queue:[/] {len(queue)} entries | [blue]Next dequeue in:[/] {next_dequeue_in:.1f}s"
            )
    except Exception:
        pass
