...
def manage_live_exits(tracker) -> None:
    ...
    try:
        ...
        enabled_map = {
            cid: cfg.symbol for cid, cfg in tracker.config.tracked_coins.items() if not cfg.disabled
        }
        ...
        for sym, pos in list(tracker.portfolio.positions.items()):
            ...
            try:
                if tracker.live_executor is not None and not tracker._live_be_armed.get(sym, False):
                    ...
                    if coin_id:
                        atr_last = (
                            (tracker.history.get(coin_id, {}) or {}).get("last", {}).get("atr")
                        )
                        coin_atr_params = tracker.atr_params_map.get(coin_id, tracker.atr_params)
                        if (
                            (coin_atr_params is not None)
                            and (atr_last is not None)
                            and (float(atr_last) > 0)
                        ):
                            sl_tmp, _tp_tmp = compute_stop_levels_atr(
                                float(pos.entry_price), float(atr_last), coin_atr_params
                            )
            ...
            if (coin_atr_params is not None) and (atr_last is not None) and (float(atr_last) > 0):
                sl_from_entry, tp_from_entry = compute_stop_levels_atr(
                    float(pos.entry_price), float(atr_last), coin_atr_params
                )
                if sl_from_entry is None or tp_from_entry is None:
                    sl_from_entry, tp_from_entry = compute_stop_levels(
                        float(pos.entry_price), tracker.risk
                    )
                trailing_level = compute_trailing_stop_atr(
                    float(pos.peak_price), float(atr_last), coin_atr_params
                ) or compute_trailing_stop(float(pos.peak_price), tracker.risk)
            else:
                sl_from_entry, tp_from_entry = compute_stop_levels(
                    float(pos.entry_price), tracker.risk
                )
            ...
            try:
                ...
                if size_usd <= 0:
                    continue
                live_order = tracker.live_executor.place_order(
                    symbol=market_pair, side="sell", size_usd=size_usd, order_type="market"
                )
                ...
                try:
                    ...
                    tracker._live_exit_backoff[sym] = {
                        "retries": retries,
                    ...
                    if delay >= 30.0:
                        log_event(
                            "live_backoff_alert",
                            {"symbol": sym, "reason": reason, "delay_sec": delay},
                        )
                        try:
                            tracker.notifier.alert(
                                "Exit Backoff",
                                f"{sym} delay {int(delay)}s (reason={reason})",
                                style="yellow",
                            )
                        except Exception:
                            pass
