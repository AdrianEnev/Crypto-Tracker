...
def validate_config(cfg: Dict[str, Any]) -> List[str]:
    ...
    if isinstance(exec_cfg, dict):
        ...
        # Exposure caps
        for k in ('max_exposure_pct','max_coin_exposure_pct','max_sector_exposure_pct','cash_floor_pct','max_portfolio_var_pct'):
        ...
        if liq:
            try:
                if 'min_daily_notional_usd' in liq:
                    ...
                    if mdn < 0:
                        errors.append("execution.liquidity_check.min_daily_notional_usd must be >= 0")
            except Exception:
