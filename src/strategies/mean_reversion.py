...
class MeanReversionStrategy(BaseStrategy):
    def __init__(self, strategy_config: dict):
        ...
        bb_cfg = (
            self.config.get("bollinger", {})
            if isinstance(self.config.get("bollinger", {}), dict)
            else {}
        )
        ...
        self.require_confluence = bool(
            self.config.get("require_confluence", bb_cfg.get("require_confluence", True))
        )
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.use_bollinger:
            mid, up, lo, width = bollinger(
                data["close"].tolist(), period=self.bb_period, stddev=self.bb_stddev
            )
            data["bb_mid"] = mid
            data["bb_up"] = up
            data["bb_lo"] = lo
