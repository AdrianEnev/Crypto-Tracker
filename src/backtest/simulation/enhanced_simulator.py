...
class EnhancedTradingSimulator:
    ...
    def _calculate_enhanced_metrics(
        self, trades: List[EnhancedTrade], equity: List[float], timeframe: str
    ) -> Dict[str, float]:
        ...
        avg_entry_slippage_bps = (
            sum(t.entry_slippage_bps for t in trades) / len(trades) if trades else 0.0
        )
        avg_exit_slippage_bps = (
            sum(t.exit_slippage_bps for t in trades) / len(trades) if trades else 0.0
        )
        ...
        cost_efficiency_score = (
            (total_execution_costs_usd / total_volume) * 100.0 if total_volume > 0 else 0.0
        )
        slippage_efficiency_score = (
            (total_slippage_usd / total_volume) * 100.0 if total_volume > 0 else 0.0
        )
