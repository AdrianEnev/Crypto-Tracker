...
def _eval_params(closes, highs, lows, times, cfg_all: Dict, coin_id: str, params: Dict[str, float], timeframe: str,
    ...
    for cid in coin_ids:
        ...
        for rsi_p, ef, es, slm, tpm, rb in combos:
            ...
            for (tr_s, tr_e, te_s, te_e) in splits:
                ...
                _ = _eval_params(
                    closes[tr_s:tr_e], highs[tr_s:tr_e], lows[tr_s:tr_e], times[tr_s:tr_e], cfg_all, cid, params, timeframe,
                ...
                # Test evaluation
                ev = _eval_params(
                    closes[te_s:te_e], highs[te_s:te_e], lows[te_s:te_e], times[te_s:te_e], cfg_all, cid, params, timeframe,
                    use_price_as_threshold=use_price_as_threshold,
                    disable_regime_filter=disable_regime_filter,
                    disable_vol_gate=disable_vol_gate,
                )
        ...
        for i, (p, lst) in enumerate(ranked[:5], start=1):
            if lst:
                ...
                print(f" {i:>2}. params={p} | PF={avg_pf:.2f} | maxDD={avg_dd:.1f}% | WR={avg_wr:.1f}% | CAGR={avg_cagr:.2f}% | MAR={avg_mar:.2f} | avgRet={avg_ret:.2f}% | trades~{avg_trades}")
            else:
        ...
        try:
            ...
            with out_path.open('w', newline='') as f:
                w = csv.writer(f)
                w.writerow(['rsi','ema_fast','ema_slow','sl_mult','tp_mult','risk_budget_pct','avg_pf','avg_dd','avg_trades','avg_win_rate','avg_cagr','avg_mar','avg_return_pct','mc_p5_return_pct','mc_p5_maxdd_pct','mc_p5_worst_week_pct'])
                for p, lst in ranked:
                    if lst:
                        avg_pf = sum(x.profit_factor for x in lst) / len(lst)
                        avg_dd = sum(x.max_drawdown for x in lst) / len(lst)
                        ...
                        avg_pf = 0.0; avg_dd = 0.0; avg_trades = 0.0; avg_wr = 0.0; avg_cagr = 0.0; avg_mar = 0.0; avg_ret = 0.0
                    # Simple Monte Carlo sequencing for this parameter set on full data
                    # Re-evaluate on full series for MC
                    ev_full = _eval_params(closes, highs, lows, times, cfg_all, cid, p, timeframe,
                                            use_price_as_threshold=use_price_as_threshold,
                                            disable_regime_filter=disable_regime_filter,
                                            disable_vol_gate=disable_vol_gate)
                    ...
                    try:
                        import random
                        rets = [t.pnl_pct() for t in simulate_on_series(
                            coin_id=cid, threshold=float((cfg_all.get('tracked_coins') or {}).get(cid, {}).get('threshold', 0.0)),
                            closes=closes, highs=highs, lows=lows,
                            rsi_period=int(p['rsi']), ema_fast=int(p['ema_fast']), ema_slow=int(p['ema_slow']),
                            atr_params=ATRRiskParams(atr_period=int((cfg_all.get('risk') or {}).get('atr', {}).get('period', 14)),
                                                     sl_mult=float(p['sl_mult']), tp_mult=float(p['tp_mult']), trail_mult=float((cfg_all.get('risk') or {}).get('atr', {}).get('trail_mult', 2.0))),
                            slippage_base_bps=int((cfg_all.get('execution') or {}).get('slippage', {}).get('base_bps', 10)),
                            slippage_k_atr_pct=float((cfg_all.get('execution') or {}).get('slippage', {}).get('k_atr_pct', 0.0)),
                            fee_bps=int(float(((cfg_all.get('execution') or {}).get('fee_tiers', [{}])[0].get('taker_bps', (cfg_all.get('execution') or {}).get('fee_bps', 5))))),
                            times=times, export_dir=None, use_regime_filter=bool((cfg_all.get('strategy') or {}).get('use_regime_filter', False)),
                            vol_gate_min_atr_pct=(cfg_all.get('strategy') or {}).get('vol_gate', {}).get('min_atr_pct'),
                            vol_gate_max_atr_pct=(cfg_all.get('strategy') or {}).get('vol_gate', {}).get('max_atr_pct'),
                            risk_budget_pct=float(p.get('risk_budget_pct', 0.0)), auto_threshold=float((cfg_all.get('decision') or {}).get('confidence_thresholds', {}).get('auto', 0.8)),
                            auto_threshold_bear=float(((cfg_all.get('decision') or {}).get('confidence_thresholds', {}).get('auto_bear', 0.8))) if ((cfg_all.get('decision') or {}).get('confidence_thresholds', {}).get('auto_bear', None) is not None) else None
                        ).trades if t.pnl_pct() is not None]
                        if rets:
                            ...
                            for _ in range(trials):
                                ...
                                if len(rets) >= 7:
                                    ...
                                    wweeks.append(0.0)
                            finals.sort(); dds.sort(); wweeks.sort()
                            mc_p5 = finals[int(0.05 * (len(finals)-1))] if finals else 0.0
                            mc_p5_maxdd = dds[int(0.95 * (len(dds)-1))] if dds else 0.0  # 95th percentile DD
                            mc_p5_worstweek = wweeks[int(0.05 * (len(wweeks)-1))] if wweeks else 0.0
                    except Exception:
                        mc_p5 = 0.0
                        mc_p5_maxdd = 0.0
                        mc_p5_worstweek = 0.0
                    w.writerow([
                        ...
                        f"{avg_pf:.4f}", f"{avg_dd:.2f}", f"{avg_trades:.2f}", f"{avg_wr:.2f}", f"{avg_cagr:.2f}", f"{avg_mar:.2f}", f"{avg_ret:.2f}", f"{mc_p5:.2f}", f"{mc_p5_maxdd:.2f}", f"{mc_p5_worstweek:.2f}"
                    ])
