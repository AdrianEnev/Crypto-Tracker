...
class DrawdownManager:
    ...
    def _add_equity_snapshot(self, equity: float, timestamp: datetime):
        ...
        try:
            # Add daily snapshot
            daily_snapshot = EquitySnapshot(timestamp=timestamp, equity=equity, period="daily")
            self.equity_history.append(daily_snapshot)

            # Keep only recent history
            cutoff_time = timestamp - timedelta(days=self.max_history_days)
            self.equity_history = [
                snapshot for snapshot in self.equity_history if snapshot.timestamp >= cutoff_time
            ]
    ...
    def calculate_drawdown_metrics(self) -> DrawdownMetrics:
        ...
        try:
            ...
            metrics.current_drawdown_pct = max(
                0.0, (self.equity_peak - current_equity) / self.equity_peak * 100.0
            )
            ...
            if self.daily_start_equity and self.daily_start_equity > 0:
                ...
                metrics.daily_drawdown_pct = (
                    max(0.0, -daily_change * 100.0) if daily_change < 0 else 0.0
                )
            ...
            if self.weekly_start_equity and self.weekly_start_equity > 0:
                weekly_change = (
                    current_equity - self.weekly_start_equity
                ) / self.weekly_start_equity
                metrics.weekly_drawdown_pct = (
                    max(0.0, -weekly_change * 100.0) if weekly_change < 0 else 0.0
                )
    ...
    def check_drawdown_limits(
        self, metrics: DrawdownMetrics, limits: Dict[str, float]
    ) -> List[RiskViolation]:
    ...
    def get_drawdown_warnings(
        self, metrics: DrawdownMetrics, warning_thresholds: Dict[str, float]
    ) -> List[str]:
