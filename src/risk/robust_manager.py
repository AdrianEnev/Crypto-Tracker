...
from .models import (
    RiskConfig, RiskLimits, RiskCheckResult, RiskStatus, RiskLevel,
    RiskViolation, RiskViolationType, ExposureMetrics, DrawdownMetrics
)
...
class RobustRiskManager:
    ...
    def _load_configuration(self):
        ...
        try:
            ...
            # Load limits
            self.config.limits.portfolio.max_exposure_per_coin_pct = robust_risk_config.get('portfolio_limits', {}).get('max_exposure_per_coin_pct', 15.0)
            self.config.limits.portfolio.max_total_exposure_pct = robust_risk_config.get('portfolio_limits', {}).get('max_total_exposure_pct', 80.0)
            self.config.limits.portfolio.max_open_positions = robust_risk_config.get('portfolio_limits', {}).get('max_open_positions', 8)
            self.config.limits.portfolio.max_correlation_exposure_pct = robust_risk_config.get('portfolio_limits', {}).get('max_correlation_exposure_pct', 25.0)
            
            self.config.limits.per_trade.max_loss_pct_equity = robust_risk_config.get('per_trade_limits', {}).get('max_loss_pct_equity', 2.0)
            self.config.limits.per_trade.max_position_size_pct = robust_risk_config.get('per_trade_limits', {}).get('max_position_size_pct', 10.0)
            self.config.limits.per_trade.min_risk_reward_ratio = robust_risk_config.get('per_trade_limits', {}).get('min_risk_reward_ratio', 1.5)
            
            self.config.limits.drawdown.daily_max_drawdown_pct = robust_risk_config.get('drawdown_limits', {}).get('daily_max_drawdown_pct', 5.0)
            self.config.limits.drawdown.weekly_max_drawdown_pct = robust_risk_config.get('drawdown_limits', {}).get('weekly_max_drawdown_pct', 12.0)
            self.config.limits.drawdown.max_drawdown_pct = robust_risk_config.get('drawdown_limits', {}).get('max_drawdown_pct', 20.0)
            self.config.limits.drawdown.kill_switch_drawdown_pct = robust_risk_config.get('drawdown_limits', {}).get('kill_switch_drawdown_pct', 15.0)
            
            self.config.limits.leverage.max_leverage = robust_risk_config.get('leverage_limits', {}).get('max_leverage', 3.0)
            self.config.limits.leverage.margin_requirement_buffer = robust_risk_config.get('leverage_limits', {}).get('margin_requirement_buffer', 1.2)
            self.config.limits.leverage.max_margin_utilization_pct = robust_risk_config.get('leverage_limits', {}).get('max_margin_utilization_pct', 75.0)
            
            self.config.limits.funding_rate.max_funding_rate_exposure = robust_risk_config.get('funding_rate_limits', {}).get('max_funding_rate_exposure', 0.01)
            self.config.limits.funding_rate.funding_rate_cost_limit_daily = robust_risk_config.get('funding_rate_limits', {}).get('funding_rate_cost_limit_daily', 0.005)
            self.config.limits.funding_rate.perpetual_exposure_limit_pct = robust_risk_config.get('funding_rate_limits', {}).get('perpetual_exposure_limit_pct', 50.0)
            
            # Load monitoring config
            monitoring_config = robust_risk_config.get('monitoring', {})
            self.config.limits.monitoring.risk_check_interval_seconds = monitoring_config.get('risk_check_interval_seconds', 30)
            self.config.limits.monitoring.alert_thresholds = monitoring_config.get('alert_thresholds', {})
    ...
    def check_pre_trade_risk(self, symbol: str, side: str, quantity: float, price: float, 
        ...
        try:
            ...
            portfolio_check = self._check_portfolio_limits(symbol, side, quantity, price, total_equity)
            if not portfolio_check.is_valid:
                return portfolio_check
            
            # Check per-trade limits
            trade_check = self._check_per_trade_limits(symbol, side, quantity, price, stop_loss, total_equity)
    ...
    def perform_risk_assessment(self, sym_to_price: Dict[str, float]) -> RiskStatus:
        ...
        try:
            ...
            exposure_metrics = self.exposure_tracker.calculate_exposure_metrics(sym_to_price, total_equity)
            ...
            exposure_violations = self.exposure_tracker.check_exposure_limits(exposure_metrics, portfolio_limits)
            ...
            drawdown_violations = self.drawdown_manager.check_drawdown_limits(drawdown_metrics, drawdown_limits)
    ...
    def _check_portfolio_limits(self, symbol: str, side: str, quantity: float, 
                              price: float, total_equity: float) -> RiskCheckResult:
        ...
        try:
            ...
            if side == 'buy' and current_positions >= self.config.limits.portfolio.max_open_positions:
    ...
    def _check_per_trade_limits(self, symbol: str, side: str, quantity: float, 
                              price: float, stop_loss: Optional[float], 
                              total_equity: float) -> RiskCheckResult:
    ...
    def _check_drawdown_limits(self) -> RiskCheckResult:
        ...
        try:
            ...
            violations = self.drawdown_manager.check_drawdown_limits(drawdown_metrics, drawdown_limits)
    ...
    def _check_leverage_limits(self, symbol: str, quantity: float, price: float, 
                             total_equity: float) -> RiskCheckResult:
    ...
    def get_risk_summary(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                'last_risk_check': self.risk_status.last_risk_check.isoformat() if self.risk_status.last_risk_check else None,
                'configuration': {
