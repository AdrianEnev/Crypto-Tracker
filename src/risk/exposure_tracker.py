...
class ExposureTracker:
    ...
    def calculate_exposure_metrics(self, sym_to_price: Dict[str, float], total_equity: float) -> ExposureMetrics:
        ...
        try:
            ...
            metrics.total_exposure_pct = (total_market_value / total_equity * 100.0) if total_equity > 0 else 0.0
            
            # Calculate per-coin exposure
            for pos in positions:
                coin_exposure_usd = pos.market_value
                coin_exposure_pct = (coin_exposure_usd / total_equity * 100.0) if total_equity > 0 else 0.0
            ...
            metrics.correlation_exposure = self._calculate_correlation_exposure(positions, total_equity)
            
            # Calculate leverage and margin utilization
            metrics.leverage_utilization = self._calculate_leverage_utilization(positions, total_equity)
    ...
    def _calculate_correlation_exposure(self, positions: List[PositionInfo], total_equity: float) -> Dict[str, float]:
            ...
            for pos in positions:
                ...
                if correlated_symbols:
                    ...
                    if correlated_exposure > 0:
                        correlation_exposure[symbol] = (correlated_exposure / total_equity * 100.0) if total_equity > 0 else 0.0
    ...
    def _calculate_leverage_utilization(self, positions: List[PositionInfo], total_equity: float) -> float:
    ...
    def _calculate_margin_utilization(self, positions: List[PositionInfo], total_equity: float) -> float:
    ...
    def check_exposure_limits(self, metrics: ExposureMetrics, limits: Dict[str, float]) -> List[RiskViolation]:
    ...
    def get_exposure_warnings(self, metrics: ExposureMetrics, warning_thresholds: Dict[str, float]) -> List[str]:
        ...
        try:
            ...
            if metrics.total_exposure_pct > exposure_warning_pct:
                warnings.append(f"Total exposure {metrics.total_exposure_pct:.2f}% approaching limit")
            ...
            if metrics.leverage_utilization > leverage_warning_pct:
                warnings.append(f"Leverage utilization {metrics.leverage_utilization:.2f}x approaching limit")
