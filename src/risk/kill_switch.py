    ...
    def _execute_emergency_actions(self):
        ...
        try:
            ...
            if self.order_manager:
                try:
                    cancelled_count = self.order_manager.cancel_all_orders(reason="kill_switch_activated")
                    # Log the emergency action
                    self._log_emergency_action(f"Cancelled {cancelled_count} orders due to kill switch")
                except Exception:
                    self._log_emergency_action("Failed to cancel orders during kill switch activation")
