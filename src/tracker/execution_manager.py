...
class ExecutionManager:
        ...
        try:
            if self.auto_trade_mode == 'live':
                ...
                if key and secret:
                    ...
                    from rich.console import Console
                    console = Console()
                    console.print(f"[yellow]Live mode requested but API keys not found for {exchange_name}. Staying in paper.[/]")
                    self.auto_trade_mode = 'paper'
    ...
    def execute_buy_order(self, symbol: str, coin_id: str, current_price: float, confidence: float) -> bool:
        ...
        try:
            ...
            log_event('buy_order_error', {
                'symbol': symbol,
                'error': str(ex)
            })
            return False
    
    def execute_sell_order(self, symbol: str, coin_id: str, current_price: float, reason: str = "manual") -> bool:
        """Execute a sell order for the given symbol."""
        try:
            position = self.portfolio_manager.get_position(symbol)
            if position is None:
                log_event('sell_order_rejected', {
                    'symbol': symbol,
                    'reason': 'no_position'
                })
                return False
    ...
    def _calculate_position_size(self, symbol: str, current_price: float, confidence: float) -> float:
        ...
        try:
            ...
            log_event('position_size_error', {
                'symbol': symbol,
                'error': str(ex)
            })
            return 0.0
    
    def _execute_paper_buy_order(self, symbol: str, coin_id: str, size_usd: float, price: float) -> bool:
        ...
        try:
            ...
            log_event('paper_buy_error', {
                'symbol': symbol,
                'error': str(ex)
            })
            return False
    
    def _execute_paper_sell_order(self, symbol: str, coin_id: str, size_usd: float, price: float) -> bool:
        ...
        try:
            ...
            log_event('paper_sell_error', {
                'symbol': symbol,
                'error': str(ex)
            })
            return False
    
    def _execute_live_buy_order(self, symbol: str, coin_id: str, size_usd: float, price: float) -> bool:
        ...
        try:
            ...
            # Execute live order
            result = self.live_executor.place_market_buy(
                symbol=market_pair,
                size_usd=size_usd
            )
            ...
            log_event('live_buy_error', {
                'symbol': symbol,
                'error': str(ex)
            })
            return False
    
    def _execute_live_sell_order(self, symbol: str, coin_id: str, size_usd: float, price: float) -> bool:
        ...
        try:
            ...
            # Execute live order
            result = self.live_executor.place_market_sell(
                symbol=market_pair,
                size_usd=size_usd
            )
            ...
            if result.success:
                ...
                self.portfolio_manager.portfolio.close_position(
                    symbol, 
                    result.filled_price if result.filled_price else price, 
                    result.order_id
                )
