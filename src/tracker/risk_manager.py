...
class RiskManager:
    ...
    def compute_stop_levels_for_symbol(
        self, symbol: str, current_price: float, coin_id: str, atr_value: Optional[float] = None
        ...
        try:
            ...
            if coin_atr_params is not None and atr_value is not None and float(atr_value) > 0:
                sl, tp = compute_stop_levels_atr(
                    float(current_price), float(atr_value), coin_atr_params
                )
