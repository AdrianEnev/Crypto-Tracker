...
class DisplayManager:
    ...
    def _display_per_coin_status(self, tracked_coins: Dict[str, Any], prices: Dict[str, float], title: str):
        ...
        try:
            ...
            for coin_id, coin_config in tracked_coins.items():
                ...
                if price is not None and coin_config.threshold > 0:
                    ...
                    change_color = "green" if change_pct > 0 else "red" if change_pct < 0 else "yellow"
                    coin_table.add_row(
    ...
    def _display_compact_status(self, tracked_coins: Dict[str, Any], prices: Dict[str, float], title: str):
    ...
    def _display_detailed_status(self, tracked_coins: Dict[str, Any], prices: Dict[str, float], title: str):
    ...
    def display_decision(self, coin_id: str, decision_data: Dict[str, Any]):
        ...
        try:
            ...
            self._display_single_decision(coin_id, {
                'action': action,
                'signal': signal,
            ...
            for coin_id, decision in decisions.items():
                ...
                if self.show_confidence:
                    if isinstance(confidence, (int, float)):
                        ...
                        row_data.append(f"[{confidence_color}]{confidence:.2f}[/{confidence_color}]")
                    else:
                        row_data.append("—")
    ...
    def _display_single_decision(self, coin_id: str, decision: Dict[str, Any]):
        ...
        try:
            ...
            if self.show_confidence and isinstance(confidence, (int, float)):
                ...
                confidence_color = "green" if confidence >= 0.8 else "yellow" if confidence >= 0.5 else "red"
                decision_parts.append(f"Confidence: [{confidence_color}]{conf_str}[/{confidence_color}]")
        ...
        try:
            ...
            for component in components:
                ...
                if 'strat=' in component:
                    self.console.print(f"  [cyan]Strategy:[/cyan] {component.replace('strat=', '')}")
                elif 'signal=' in component:
                    self.console.print(f"  [yellow]Signal:[/yellow] {component.replace('signal=', '')}")
