...
class PriceManager:
    ...
    def __init__(self, config_manager, app_config: Any):
        ...
        self.fetcher = PriceFetcher(
            base_url=app_config.api_base_url, timeout=app_config.api_timeout
        )
    ...
    def _preload_history(self):
        ...
        try:
            ...
            for coin_id, coin_cfg in self.app_config.tracked_coins.items():
                ...
                try:
                    ...
                    if provider == "ccxt":
                        ...
                        candles = get_candles(
                            cg_key, timeframe=tf, days=days, cache_dir=cache_dir, use_cache=True
                        )
            ...
            if loaded > 0:
                ...
                console = Console()
                console.print(
                    f"[blue]Preloaded history for {loaded} assets (tf={tf}, days={days}).[/]"
                )
    ...
    def get_aggregated_prices(
        self, enabled_coins: Dict[str, str], cg_ids: Optional[Dict[str, str]] = None
    ...
    def refresh_history_tail(self, coin_id: str) -> bool:
        ...
        try:
            ...
            if provider == "ccxt":
                ...
                market = (
                    cdata.get("market")
                    or f"{self.app_config.tracked_coins[coin_id].symbol.upper()}/USDT"
                )
