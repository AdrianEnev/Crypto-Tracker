...
class VolumeBasedSlippage:
    ...
    def calculate_slippage(self, context: SlippageContext) -> SlippageResult:
        ...
        return SlippageResult(
            ...
            * (
                1 + (total_slippage / 10000.0)
                if context.side == "buy"
                else 1 - (total_slippage / 10000.0)
            ),
    ...
    def _calculate_static_slippage(self, context: SlippageContext) -> SlippageResult:
        ...
        return SlippageResult(
            ...
            effective_price=reference_price
            * (
                1 + (self.base_slippage_bps / 10000.0)
                if context.side == "buy"
                else 1 - (self.base_slippage_bps / 10000.0)
            ),
            reference_price=reference_price,
    ...
    def calculate_twap_impact(
        self, total_quantity: float, num_slices: int, context: SlippageContext
    ) -> List[SlippageResult]:
    ...
    def get_volume_profile(
        self, context: SlippageContext, order_sizes: List[float]
    ) -> Dict[float, float]:
