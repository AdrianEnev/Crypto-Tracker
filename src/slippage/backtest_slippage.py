...
class SlippageStats:
    ...
    def add_trade(
        self, slippage_result: SlippageResult, trade_value_usd: float, quantity: float
    ) -> None:
        ...
        for condition, slippage in self.slippage_by_condition.items():
            ...
            breakdown[condition.value] = {
                ...
                "percentage_of_trades": (
                    trades / (self.market_order_count + self.limit_order_count)
                )
                * 100,
            }
        return breakdown
...
class BacktestSlippageCalculator:
    ...
    def __init__(self, slippage_model: SlippageType = SlippageType.DEPTH_BASED):
        ...
        self.slippage_history: List[Tuple[datetime, SlippageResult, float, float]] = (
            []
        )  # (timestamp, result, trade_value, quantity)
    ...
    def calculate_slippage_with_tracking(
        self, context: SlippageContext, trade_value_usd: float, quantity: float
    ) -> SlippageResult:
        """
    ...
    ) -> List[Tuple[datetime, SlippageResult, float, float]]:
    ...
    def optimize_order_sizing(
        self, total_quantity: float, context: SlippageContext, max_slippage_bps: float
    ) -> Dict[str, any]:
        """Find optimal order sizing strategy to minimize slippage."""
        if self.slippage_model == SlippageType.DEPTH_BASED and hasattr(
            self.calculator, "get_optimal_order_size"
        ):
            optimal_size = self.calculator.get_optimal_order_size(
                context.order_book.get_market_depth(), context.side, max_slippage_bps
            )
            num_slices = max(1, int(total_quantity / optimal_size))
        elif self.slippage_model == SlippageType.VOLUME_BASED and hasattr(
            self.calculator, "get_optimal_order_size"
        ):
