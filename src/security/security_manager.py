...
class SecurityManager:
    ...
    def validate_exchange_api_key(self, exchange_name: str, api_key: str, secret: str) -> APIKeyValidationResult:
    ...
    def is_trading_safe(self, exchange_name: str, api_key: str, secret: str) -> bool:
        ...
        if result.safety_status in [SafetyStatus.SAFE, SafetyStatus.WARNING]:
            if result.warnings:
                self.logger.warning(f"Trading allowed for {exchange_name} with warnings: {result.warnings}")
            return True
