...
class BacktestFeeStats:
    ...
    def add_trade(self, fee_breakdown: FeeBreakdown, trade_value_usd: float) -> None:
        ...
        if self.maker_trade_count > 0:
            self.avg_maker_fee_bps = (self.maker_fees_usd / 
                                    sum(f.trading_fee_usd for f in self.fee_history if f.fee_type_used.value == "maker")) * 10000
        
        if self.taker_trade_count > 0:
            self.avg_taker_fee_bps = (self.taker_fees_usd / 
                                    sum(f.trading_fee_usd for f in self.fee_history if f.fee_type_used.value == "taker")) * 10000
...
class BacktestFeeCalculator(FeeCalculator):
    ...
    def __init__(
        ...
        self.fee_history: List[Tuple[datetime, FeeBreakdown, float]] = []  # (timestamp, fees, trade_value)
    ...
    def get_fee_history(self, start_time: Optional[datetime] = None, end_time: Optional[datetime] = None) -> List[Tuple[datetime, FeeBreakdown, float]]:
    ...
    def compare_exchanges(self, order_value_usd: float, symbol: str, side: str, order_type: str) -> Dict[str, FeeBreakdown]:
    ...
    def optimize_for_maker_fees(self, target_volume_usd: float, symbol: str) -> Dict[str, Dict[str, any]]:
