...
class OrderStateMachine:
    ...
    def transition(self, order: Order, new_state: OrderState, reason: Optional[str] = None) -> bool:
        ...
        if not self.can_transition(order.state, new_state):
            ...
            validation_result.add_error(f"Invalid state transition from {order.state.value} to {new_state.value}")
    ...
    def _record_transition(self, order_id: str, from_state: OrderState, to_state: OrderState, reason: Optional[str]):
