...
class ReconciliationResult:
    ...
    def add_discrepancy(self, order_id: str, discrepancy_type: str, 
                       local_value: Any, exchange_value: Any, severity: str = "warning") -> None:
...
class OrderCancellationManager:
    ...
    def cancel_order(self, order_id: str, reason: str = "manual", force: bool = False) -> CancellationResult:
    ...
    def cancel_all_orders(self, symbol: Optional[str] = None, reason: str = "bulk_cancel") -> List[CancellationResult]:
    ...
    def cancel_orders_by_strategy(self, strategy_id: str, reason: str = "strategy_cancel") -> List[CancellationResult]:
    ...
    def _execute_cancellation(self, order: Order, request: CancellationRequest) -> CancellationResult:
        ...
        try:
            ...
            if success:
                ...
                self.order_manager.state_machine.transition(order, OrderState.CANCELED, request.reason)
                ...
                return CancellationResult(
                    order_id=order.id,
                    success=False,
                    error_message="Exchange cancellation failed"
                )
...
class OrderReconciler:
    ...
    def reconcile_order(self, order_id: str) -> ReconciliationResult:
        ...
        try:
            ...
            if exchange_order:
                ...
                result.add_discrepancy(order_id, "missing_on_exchange", order.state.value, None, "error")
    ...
    def _reconcile_exchange_orders(self, exchange: str, orders: List[Order], result: ReconciliationResult) -> None:
    ...
    def _reconcile_order_states(self, local_order: Order, exchange_order: Order, result: ReconciliationResult) -> None:
        ...
        if (local_order.average_fill_price is not None and 
            exchange_order.average_fill_price is not None and
            abs(local_order.average_fill_price - exchange_order.average_fill_price) > 0.001):
            result.add_discrepancy(
                local_order.id,
    ...
    def _handle_critical_discrepancy(self, discrepancy: ReconciliationDiscrepancy) -> None:
        ...
        if discrepancy.discrepancy_type == "missing_on_exchange":
            ...
            if order and order.is_active:
                self.order_manager.state_machine.transition(order, OrderState.CANCELED, "reconciliation")
            ...
            if order:
                ...
                self.logger.warning(f"State mismatch for order {order_id}: {discrepancy.local_value} vs {discrepancy.exchange_value}")
