...
def main():
    ...
    try:
        ...
        if getattr(tracker, "_manage_live_exits_orig", None) is None and hasattr(tracker, "_manage_live_exits"):
            tracker._manage_live_exits_orig = tracker._manage_live_exits
        if getattr(tracker, "_reconcile_live_protection_orig", None) is None and hasattr(tracker, "_reconcile_live_protection"):
            tracker._reconcile_live_protection_orig = tracker._reconcile_live_protection
        if getattr(tracker, "_cancel_orphan_sell_orders_orig", None) is None and hasattr(tracker, "_cancel_orphan_sell_orders"):
    ...
    try:
        ...
        if getattr(tracker, "_preload_history_orig", None) is None and hasattr(tracker, "_preload_history"):
            tracker._preload_history_orig = tracker._preload_history
        if getattr(tracker, "_refresh_history_tail_orig", None) is None and hasattr(tracker, "_refresh_history_tail"):
    ...
    try:
            ...
            try:
                ...
                for cid, pdata in (aggregated or {}).items():
                    try:
                        ...
                        continue
                eq = tracker.compute_equity(sym_to_price) if hasattr(tracker, 'compute_equity') else 0.0
        ...
        def _dd_guard_job():
            try:
                ...
                if dd >= float(ks):
                    ...
                    try:
                        tracker.notifier.alert('Kill Switch', f"DD {dd:.2f}% >= {float(ks):.2f}% — blocking new entries", style='red')
                    except Exception:
                        pass
                else:
                    # Only clear if previously active and recover below threshold by some margin
                    if getattr(tracker, 'kill_switch_active', False) and dd < max(0.0, float(ks) - 0.5):
                        tracker.kill_switch_active = False
                        try:
                            tracker.notifier.alert('Kill Switch', f"Recovered DD {dd:.2f}%, entries allowed", style='green')
                        except Exception:
                            pass
            except Exception:
                pass
        ...
        def _position_snapshot_job():
            try:
                if getattr(tracker, 'store', None) is None or getattr(tracker, 'portfolio', None) is None:
                    return
