...
class JSONLOrderBookStorage(OrderBookStorage):
    ...
    def get_snapshots(
        ...
        while current_date <= end_date:
            file_path = self._get_snapshot_file(symbol, datetime.combine(current_date, datetime.min.time()))
    ...
    def get_events(
        ...
        while current_date <= end_date:
            file_path = self._get_event_file(symbol, datetime.combine(current_date, datetime.min.time()))
...
class SQLiteOrderBookStorage(OrderBookStorage):
    ...
    def _create_tables(self):
        ...
        # Create indexes
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_snapshots_symbol_timestamp ON snapshots(symbol, timestamp)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_events_symbol_timestamp ON events(symbol, timestamp)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_metrics_symbol_timestamp ON metrics(symbol, timestamp)")
    ...
    def store_snapshot(self, snapshot: OrderBookSnapshot) -> bool:
        ...
        try:
            ...
            bids_json = json.dumps([(level.price, level.quantity) for level in snapshot.bids.levels])
            asks_json = json.dumps([(level.price, level.quantity) for level in snapshot.asks.levels])
    ...
    def get_snapshots(
        ...
        for row in cursor.fetchall():
            try:
                timestamp_str, bids_json, asks_json, last_trade_price, last_trade_quantity, last_trade_id, sequence_number = row
    ...
    def get_events(
        ...
        for row in cursor.fetchall():
            try:
                timestamp_str, event_type, bids_json, asks_json, trade_price, trade_quantity, trade_side, sequence_number = row
    ...
    def get_metrics(
        ...
        for row in cursor.fetchall():
            try:
                (timestamp_str, total_bid_liquidity, total_ask_liquidity,
                 weighted_bid_price, weighted_ask_price, spread, spread_bps,
                 mid_price, depth_5_levels, depth_10_levels, depth_20_levels,
                 bid_ask_ratio, order_imbalance) = row
