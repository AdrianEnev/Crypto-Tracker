...
class OrderBookFetcher:
    ...
    def __init__(
        self, exchange_name: str, api_key: Optional[str] = None, secret: Optional[str] = None
    ):
        ...
        while True:
            try:
                ...
                if last_snapshot:
                    ...
                    bids_update = self._calculate_bid_updates(
                        last_snapshot.bids.levels, current_snapshot.bids.levels
                    )
                    asks_update = self._calculate_ask_updates(
                        last_snapshot.asks.levels, current_snapshot.asks.levels
                    )
    ...
    def _calculate_bid_updates(
        self, old_levels: List, new_levels: List
    ...
    def fetch_historical_order_books(
        self, symbol: str, start_time: datetime, end_time: datetime, interval_minutes: int = 1
...
class MultiExchangeOrderBookFetcher:
    ...
    def fetch_all_exchanges_order_books(
        self, symbol: str
